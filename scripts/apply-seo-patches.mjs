...
// The real Clarity project ID for artisanpeintre-fenouillet.fr, retrieved from
...
function gateGA4OnConsent(html) {
  ...
  // Fenouillet format : <script>window.addEventListener('load', function(){...})</script>
  const gaRe = /<script>\s*window\.addEventListener\('load', function\(\) \{\s*var s = document\.createElement\('script'\);\s*s\.src = 'https:\/\/www\.googletagmanager\.com\/gtag\/js\?id=([^']+)';\s*s\.async = true;\s*document\.head\.appendChild\(s\);\s*s\.onload = function\(\) \{\s*window\.dataLayer = window\.dataLayer \|\| \[\];\s*function gtag\(\)\{dataLayer\.push\(arguments\);\}\s*gtag\('js', new Date\(\)\);\s*gtag\('config', '[^']+'\);\s*\};\s*\}\);\s*<\/script>/;

  if (!gaRe.test(html)) return html;
  return html.replace(gaRe, (_m, gaId) =>
    `<script>
    // ── COOKIE-CONSENT GATED GA4 ── only loads after user accepts

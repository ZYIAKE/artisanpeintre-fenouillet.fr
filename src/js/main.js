 ...
 * Artisan Peintre Fenouillet — VG Peintures et Rénovations
 */
...
;(function () {
  ...
  document.querySelectorAll('.nav-links a:not(.nav-cta)').forEach(a => {
    ...
    if (
      page.match(/^peinture|^ravalement|^papier|^enduit/) &&
      href === 'prestations.html'
    ) {
      a.classList.add('active')
    }
    // Pages de ville → activer le lien "Zones"
    if (
      location.pathname.includes('/villes/') &&
      href === 'zone-intervention.html'
    ) {
      a.classList.add('active')
    }
  })
...
document.querySelectorAll('.real-carousel-wrapper').forEach(wrapper => {
  const track = wrapper.querySelector('.real-carousel-track')
  const slides = wrapper.querySelectorAll('.real-carousel-slide')
  const prevBtn = wrapper.parentElement.querySelector('.real-carousel-prev')
  const nextBtn = wrapper.parentElement.querySelector('.real-carousel-next')
  ...
  function getMaxIndex() {
    return Math.max(0, slides.length - getPerPage())
  }

  function update() {
    const perPage = getPerPage()
    const gap = 19.2 // 1.2rem
    const slideWidth = (track.offsetWidth - gap * (perPage - 1)) / perPage
    track.style.transform = `translateX(-${current * (slideWidth + gap)}px)`
    if (prevBtn) prevBtn.style.opacity = current === 0 ? '.3' : '1'
    if (nextBtn) nextBtn.style.opacity = current >= getMaxIndex() ? '.3' : '1'
  }
